...
沙盒是一種安全隔離機制，讓網頁程式碼在受限的環境中執行，無法直接存取作業系統資源。即使網站含有惡意程式碼，影響範圍也被限制在沙盒內。
...
| 被限制的行為 | 說明 |
|-------------|------|
| **檔案系統存取** | 網頁無法讀寫電腦上的檔案（除非使用者主動授權，如 `<input type="file">`） |
| **跨分頁存取** | Tab A 的程式碼無法讀取 Tab B 的資料（Same-Origin Policy） |
| **系統呼叫** | 無法直接呼叫 OS API（如開啟程式、修改系統設定） |
| **記憶體隔離** | 每個 renderer process 有獨立記憶體空間，一個分頁崩潰不影響其他分頁 |
| **網路限制** | 受 CORS 政策約束，無法任意向其他伺服器發送請求 |

Chrome 為每個分頁建立獨立的 renderer process，因此一個分頁當掉其他分頁不受影響，但也因此更耗記憶體。

**與前端開發的關聯**

沙盒機制解釋了前端開發中常見的限制：
- `localStorage` / `sessionStorage` 受同源政策限制，不同 origin 的資料互相看不到
- `fetch` 跨域請求需要伺服器回傳正確的 CORS headers
- `iframe` 有 `sandbox` 屬性可以進一步限制嵌入頁面的權限
