...
export function UsageIntegrationPlatformTabs() {
  const router = useRouter();
  const { platform } = useParams<{ platform: PlatformsValue }>();

  if (!isValidPlatform(platform)) {
    redirect(
      ORG_SETTINGS_PATHS.usageIntegration.pathname.replace('[platform]', PlatformsValue.GCP)
    );
  }
  ...
  const handleTabChange = (_event: SyntheticEvent, platformVal: PlatformsValue) => {
    ...
    router.push(ORG_SETTINGS_PATHS.usageIntegration.pathname.replace('[platform]', platformVal));
  };
