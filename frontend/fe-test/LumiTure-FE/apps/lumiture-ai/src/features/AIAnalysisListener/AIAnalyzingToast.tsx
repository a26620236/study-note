...
const LABELS = {
  ...
  toastLink: 'View Status',
};

interface AIAnalyzingToastProps {
  id: ToastT['id'];
  platform: PlatformsValue;
}

export function AIAnalyzingToast({ id, platform }: AIAnalyzingToastProps) {
  const theme = useTheme();
  const router = useRouter();
  const pathname = usePathname();
  const { addToastId, removeToastId } = useAIAnalysisStore();
  ...
  }, [pathname, targetPath, id, removeToastId, platform]);
