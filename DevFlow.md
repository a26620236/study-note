...
## User Flow 的職責歸屬：PM vs 設計師

### 傳統分工

**PM 負責：**
- 核心流程與業務邏輯：使用者要完成什麼目標、有哪些步驟、每步的條件與分支（如：未登入 → 導向登入頁）
- 邊界案例與錯誤處理：例如付款失敗、庫存不足等場景
- 驗收標準：定義「完成」的條件

**設計師負責：**
- 互動細節與體驗：具體的畫面轉場、元件行為、微互動
- 資訊架構：內容如何呈現、導航結構
- 可用性考量：根據設計原則優化流程，減少使用者認知負擔

> 簡單來說：PM 定義的是「業務層的 user flow」（邏輯與規則），設計師產出的是「體驗層的 user flow」（畫面與互動）。兩者是不同粒度的東西。

### 常見協作模式

| 模式 | 適用情境 |
|------|---------|
| PM 先出粗略流程 → 設計師細化 | 最常見，PM 定義「做什麼」，設計師決定「怎麼做」 |
| 設計師主導 | 設計驅動的團隊，或體驗改善型需求 |
| 共同產出 | 複雜功能，兩邊一起在白板上畫出來 |

### 設計師主導 User Flow 的做法

也有團隊讓設計師主導 User Flow，PM 只定義功能需求：

**PM 只負責：**
- 功能需求（Feature Requirements）：「我們需要一個結帳功能」
- 商業目標與 KPI：「轉換率要提升 X%」
- 限制條件：「必須支援信用卡和超商付款」
...
| 情境 | 原因 |
|------|------|
| 設計驅動的公司（Apple、Airbnb 風格） | 體驗是核心競爭力，設計師話語權大 |
| 設計師同時具備 UX Research 能力 | 他們比 PM 更了解使用者行為 |
| PM 偏向 Business/Strategy 型 | PM 專注市場、商業模式，不介入互動細節 |
| 小團隊 / 新創 | 人少，設計師自然扛更多 |

**潛在風險：**
- **設計師可能缺乏業務全貌**：不清楚後端限制、第三方 API 的行為、法規要求等，導致設計出「技術上做不到」或「業務上不合規」的流程
- **邊界案例容易遺漏**：錯誤處理、權限控制、併發情境這些通常是 PM 或工程師比較敏感的
- **責任歸屬模糊**：流程出問題時，是需求沒寫清楚還是設計有誤？

**建議配套：**
1. PM 提供完整的 Acceptance Criteria — 不定義流程，但把「必須滿足的條件」列清楚
2. 設計師產出 User Flow 後，PM + 工程師做 Review — 確保業務邏輯和技術可行性沒問題
3. 工程師早期參與 — 在流程定稿前就讓工程師看過，避免後期才發現做不了

> 結論：可以讓設計師主導 User Flow，但前提是設計師有足夠的業務理解力，且團隊有明確的 review 機制來補上 PM 不介入流程設計所留下的空缺。這不是「誰該做」的問題，而是「團隊怎麼確保流程的完整性」的問題。
